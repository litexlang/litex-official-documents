...
prove forall x, y Z: x != y => g(x) != g(y):
    forall x Z: (7 * x + 1) % 7 = ((7 * x) % 7 + 1 % 7) % 7 = (0 + 1) % 7 = 1
    forall x Z: (7 * (-x) + 2) % 7 = ((7 * (-x)) % 7 + 2 % 7) % 7 = (0 + 2) % 7 = 2
...
witness f: f2 fn(N)Z st $is_injective_fn(N, Z, f2)
witness g: g2 fn(Z)N st $is_injective_fn(Z, N, g2)
